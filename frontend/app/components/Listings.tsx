...
export default function Listings() {
  ...
  const currentItems = fetchedbusiness.slice(indexOfFirstItem, indexOfLastItem);

  const paginate = (pageNumber: number) => setCurrentPage(pageNumber);

  const nextPage = () => {
    if (currentPage < Math.ceil(fetchedbusiness.length / itemsPerPage)) {
  ...
  return (
    <ul role="list" className="divide-y divide-gray-100 space-y-3">
      {currentItems.map((business) => (
        <li key={business.id} className="grid grid-cols-2 sm:grid-cols-1 border p-5 rounded-md">
          <div className="flex flex-row flex-wrap justify-between pl-2 pr-2 sm:col-span-1">
            <div className="col-span-2 sm:col-span-1">
              ...
              <Link href={`/business/${business.id}`} className="flex flex-row items-center justify-center gap-1.5 group">
      ...
      <div className="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
        <div className="flex flex-1 justify-between sm:hidden">
          ...
          <button
            onClick={nextPage}
            disabled={currentPage === Math.ceil(fetchedbusiness.length / itemsPerPage)}
        ...
        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-gray-700">
              ...
              <span className="font-medium">{Math.min(indexOfLastItem, fetchedbusiness.length)}</span> of{' '}
              <span className="font-medium">{fetchedbusiness.length}</span> results
            </p>
          </div>
          <div>
            <nav aria-label="Pagination" className="isolate inline-flex -space-x-px rounded-md shadow-xs">
              ...
              {Array.from({ length: Math.ceil(fetchedbusiness.length / itemsPerPage) }, (_, i) => (
                <button
                  key={i + 1}
